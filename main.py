...
def main():
    ...
    group.add_argument(
        "-fa", "--fadd", help="Performs addition", action="store_true")
    group.add_argument(
        "-fs", "--fsub", help="Performs subtraction", action="store_true")
    group.add_argument(
        "-fd", "--fdiv", help="Performs division", action="store_true")
    group.add_argument("-fm", "--fmulti",
                       help="Performs multiplication", action="store_true")
    group.add_argument(
        "-fp", "--fperc", help="Performs percentage calculation", action="store_true")
    group.add_argument(
        "-ff", "--ffact", help="Performs factorial calculation", action="store_true")
    group.add_argument("-fsq", "--fsqrt",
                       help="Performs square root", action="store_true")
    group.add_argument(
        "-fpw", "--fpow", help="Performs power operation", action="store_true")
    group.add_argument(
        "-fl", "--flog", help="Performs base-10 logarithm", action="store_true")
    group.add_argument(
        "-fsi", "--fsin", help="Performs sine operation", action="store_true")
    group.add_argument(
        "-fco", "--fcos", help="Performs cosine operation", action="store_true")
    group.add_argument(
        "-fta", "--ftan", help="Performs tangent operation", action="store_true")
    group.add_argument(
        "-fdg", "--fdeg", help="Converts angle from radians to degrees", action="store_true")
    group.add_argument(
        "-frd", "--frad", help="Converts angle from degrees to radians", action="store_true")

    parser.add_argument("num1", help="Number1 to calculate", type=int)
    parser.add_argument(
        "num2", nargs='?', help="Number2 to calculate, this is optional", type=int)
    ...
    if args.fadd:
        ...
        print(
            "The base-10 logarithmic value of {} is {}".format(args.num1, (log(args.num1))))
    elif args.fsin:
